...
def run_test_3(bit_string, bit_string_length):
    ...
    observedNumber = 1
    for i in range(1, len(bit_string_length)):
        if bit_string[i] != bit_string[i - 1]:
            observedNumber += 1
    p_Value = sp.erfc(abs(observedNimber - 2 * bit_string_length * proportionOf1 * (1 - proportionOf1)) / (2 * (2 * bit_string_length) ** 0.5 * proportionOf1 * ( 1 - proportionOf1)))
    frequency_test_conclusion = (p_Value > 0.01)
    if frequency_test_conclusion:
